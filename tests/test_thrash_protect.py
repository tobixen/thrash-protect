...
class TestOOMProtection:
    ...
    def test_predictor_declining_memory_short_window(self):
        """Test ETA calculation with declining memory over the short-scale window.

        With observation_window=60, the short scale is 60/12=5s.
        Two observations 5s apart will be matched by the short-scale window.
        Short-scale horizon = 5 * (600/60) = 50s.
        """
        predictor = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)
        ...
        # 5 seconds later: available = 3M + 5M * 2 = 13M (declined by 3M in 5s)
        # decline_rate = 3M / 5s = 600000 kB/s
        # eta = 13M / 600000 ≈ 21.67 seconds
        # Short-scale horizon = 50s, so 21.67 < 50 -> triggers
        with patch("thrash_protect.read_meminfo", return_value=(3000000, 5000000, 16000000, 8000000)):
            with patch("time.time", return_value=1005.0):
                eta = predictor.update_and_predict()
                assert eta is not None
                assert abs(eta - 21.67) < 0.1

    def test_predictor_declining_memory_main_window(self):
        """Test ETA calculation using the main (60s) observation window."""
        predictor = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)
        ...
        # Fill some intermediate observations (memory declining steadily)
        for t in range(1010, 1060, 10):
            frac = (t - 1000) / 60
            mem = int(4000000 - 1000000 * frac)
            swap = int(6000000 - 500000 * frac)
            with patch("thrash_protect.read_meminfo", return_value=(mem, swap, 16000000, 8000000)):
                with patch("time.time", return_value=float(t)):
                    predictor.update_and_predict()

        # At t=1060 (60s after first): available should trigger main window
        # available at t=1000: 4M + 6M*2 = 16M
        # available at t=1060: 3M + 5.5M*2 = 14M (approx)
        with patch("thrash_protect.read_meminfo", return_value=(3000000, 5500000, 16000000, 8000000)):
            with patch("time.time", return_value=1060.0):
                eta = predictor.update_and_predict()
                # Main window: decline from 16M to 14M over 60s
                # decline_rate = 2M / 60 = 33333 kB/s
                # eta = 14M / 33333 ≈ 420s
                # Main horizon = 600s, so 420 < 600 -> triggers
                assert eta is not None
                assert eta < 600

    def test_predictor_slow_decline_no_trigger(self):
        """Test that a very slow decline doesn't trigger any window.

        A tiny decline over 60s should produce an ETA beyond even the main horizon.
        """
        predictor = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)

        # large available: 8M + 8M*2 = 24M
        with patch("thrash_protect.read_meminfo", return_value=(8000000, 8000000, 16000000, 8000000)):
            with patch("time.time", return_value=1000.0):
                predictor.update_and_predict()

        # 60s later: tiny decline of 10kB total
        # decline_rate = 10 / 60 = 0.167 kB/s
        # eta = ~24M / 0.167 ≈ 144M seconds -> way beyond 600s horizon
    ...
    def test_predictor_swap_weight_ssd_vs_hdd(self):
        ...
        """
        predictor_ssd = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)
        predictor_hdd = thrash_protect.MemoryExhaustionPredictor(swap_weight=4.0, observation_window=60, horizon=600)
        ...
        # 5s later: swap dropped from 500k to 250k
        # SSD: 2M -> 1.5M (decline 500k in 5s), eta = 1.5M/100k = 15s
        # HDD: 3M -> 2M (decline 1M in 5s), eta = 2M/200k = 10s
    ...
    def test_predictor_should_freeze_rapid_decline(self):
        """Test should_freeze returns True on rapid decline."""
        predictor = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)
        ...
        # Rapid decline in 5s: available goes from 4M to 1.5M
        # Short window: decline_rate = 2.5M/5s = 500k/s, eta = 1.5M/500k = 3s
        # Short horizon = 50s, so 3 < 50 -> triggers
        with patch("thrash_protect.read_meminfo", return_value=(500000, 500000, 16000000, 8000000)):
            with patch("time.time", return_value=1005.0):
                result = predictor.should_freeze()
                assert result is True

    def test_predictor_should_not_freeze_tiny_decline(self):
        """Test should_freeze returns False when decline is negligible."""
        predictor = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)
        ...
        # Tiny decline: 10kB in 5s -> ETA is huge, beyond any horizon
        with patch("thrash_protect.read_meminfo", return_value=(7999990, 8000000, 16000000, 8000000)):
            with patch("time.time", return_value=1005.0):
                result = predictor.should_freeze()
                assert result is False

    def test_predictor_multi_scale_only_short_triggers(self):
        """Test that a rapid short-term decline triggers even without long-term data.

        When the predictor has only been running for a few seconds, only the
        short-scale window has data. A rapid decline should still trigger.
        """
        predictor = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)

        # Two observations 3s apart, sharp decline (rogue process eating RAM)
        with patch("thrash_protect.read_meminfo", return_value=(1000000, 200000, 16000000, 8000000)):
            with patch("time.time", return_value=1000.0):
                predictor.update_and_predict()

        # available: 1M + 200k*2 = 1.4M -> 500k + 100k*2 = 700k in 3s
        # decline = 700k in 3s, rate = 233k/s, eta = 700k/233k = 3s
        # Short window horizon = 50s, 3 < 50 -> triggers
        with patch("thrash_protect.read_meminfo", return_value=(500000, 100000, 16000000, 8000000)):
            with patch("time.time", return_value=1003.0):
                eta = predictor.update_and_predict()
                assert eta is not None
                assert eta < 5  # should be ~3s

    def test_predictor_old_observations_pruned(self):
        """Test that observations older than max_age are pruned."""
        predictor = thrash_protect.MemoryExhaustionPredictor(swap_weight=2.0, observation_window=60, horizon=600)
        ...
        # Add observation at t=100 (well beyond max_age of 90s)
        with patch("thrash_protect.read_meminfo", return_value=(4000000, 6000000, 16000000, 8000000)):
    ...
    def test_oom_cli_arguments(self):
        ...
        args = parser.parse_args(["--oom-observation-window", "30"])
        assert args.oom_observation_window == 30
...
class TestCgroupFreezeBugFixes:
    ...
    def test_unfreeze_cgroup_failure_reinserts(self, unlink, mock_open, mock_unfreeze):
        ...
        cgroup_path = "/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/tmux-spawn-abc.scope"
        thrash_protect._tp.frozen_items = [("cgroup", cgroup_path, (100,))]
        thrash_protect._tp.frozen_cgroup_paths.add(cgroup_path)
        ...
        assert len(thrash_protect._tp.frozen_items) == 1
        assert thrash_protect._tp.frozen_items[0][1] == cgroup_path
        # num_unfreezes should NOT have been incremented
        assert thrash_protect._tp.num_unfreezes == 0
        # cgroup_path should still be in frozen_cgroup_paths
        assert cgroup_path in thrash_protect._tp.frozen_cgroup_paths
    ...
    def test_unfreeze_cgroup_success_removes(self, unlink, mock_open, mock_unfreeze):
        ...
        assert len(thrash_protect._tp.frozen_items) == 0
        # num_unfreezes should have been incremented
        assert thrash_protect._tp.num_unfreezes == 1
        # cgroup_path should be removed from frozen_cgroup_paths
        assert cgroup_path not in thrash_protect._tp.frozen_cgroup_paths
