...
cmd_whitelist = ['sshd', 'bash', 'screen', 'SCREEN', 'mutt', 'ssh', 'xterm', 'rxvt', 'urxvt']
...
## smtp etc.
def log_frozen(pid):
    logfile = open("/var/log/thrash-protect.log", 'a')
    logfile.write("%s - frozen pid %s - frozen list: %s\n" % (time.time(), pid, frozen_pids))
    logfile.close()
    logfile = open("/tmp/thrash-protect-frozen-pid-list", "w")
